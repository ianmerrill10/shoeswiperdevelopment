          ...
          <div className="inline-flex rounded-full bg-white/5 p-1 shadow-lg shadow-purple-500/40 backdrop-blur">
            {tabs.map((tab) => (
              <button
                ...
                className={`relative flex-1 rounded-full px-4 py-1.5 text-xs font-medium transition ${activeTab === tab.id
                    ? "text-slate-900"
            ...
            <div className="flex flex-wrap items-center gap-2 rounded-full bg-white/5 p-1 shadow-inner shadow-purple-500/30 backdrop-blur">
              {filters.map((filter) => (
                <button
                  ...
                  className={`rounded-full px-3 py-1 text-xs font-medium transition ${activeFilter === filter.id
            ...
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center rounded-3xl border border-dashed border-slate-600/60 bg-slate-900/30 px-6 py-16 text-center backdrop-blur">
                ...
                <p className="mt-2 max-w-sm text-sm text-slate-300/75">
                  Mint your first pair in the <button onClick={() => setActiveTab('create')} className="font-medium text-violet-300 hover:text-white underline decoration-violet-500/50 underline-offset-4 transition">Create</button> tab, or
                  switch back to <button onClick={() => setActiveTab('explore')} className="font-medium text-violet-300 hover:text-white underline decoration-violet-500/50 underline-offset-4 transition">Explore</button> to browse the global
                  feed.
                </p>
              </div>
            ) : (
