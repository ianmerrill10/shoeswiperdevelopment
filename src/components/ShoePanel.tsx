// ============================================
// FILE: src/components/ShoePanel.tsx
// COMPLETE 3D/360° VIEW PANEL COMPONENT
// ============================================
...
const ShoePanel: React.FC<ShoePanelProps> = ({ shoe, isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      {/* Panel */}
      <div
        className={`fixed top-0 left-0 h-full w-full max-w-md bg-zinc-950 z-50 transform transition-transform duration-300 ease-out overflow-y-auto ${isOpen ? 'translate-x-0' : '-translate-x-full'
          }`}
      >
        {/* Header */}
          ...
          <div className="grid grid-cols-4 gap-2">
            {sizes.map((size) => (
              <button
                ...
                className={`py-3 rounded-lg font-bold text-sm transition-colors ${selectedSize === size
        ...
        <div className="p-4 space-y-3 pb-8">
          ...
          <div className="flex gap-3">
            <button
              ...
              className={`flex-1 font-bold py-3 rounded-xl flex items-center justify-center gap-2 transition-colors ${isInCloset
                  ? 'bg-orange-500 text-white'
        ...
        {/* Share Success Toast */}
        <div
          className={`fixed bottom-8 left-1/2 -translate-x-1/2 bg-zinc-700 text-white px-4 py-3 rounded-xl flex items-center gap-2 shadow-lg z-[60] transition-all duration-300 ${showShareToast ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'
            }`}
        >
