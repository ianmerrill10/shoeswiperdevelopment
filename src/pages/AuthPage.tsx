...
import { useRateLimit, RATE_LIMITS } from '../lib/rateLimiting';
...
const AuthPage: React.FC = () => {
  ...
  const { checkLimit, isLimited } = useRateLimit(RATE_LIMITS.auth);
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    // Check rate limit
    if (!checkLimit()) {
      setError(RATE_LIMITS.auth.message);
      return;
    }
      ...
      <div className="w-full max-w-md bg-zinc-900 rounded-2xl border border-zinc-800 p-6">
        ...
        <div className="flex mb-6 bg-zinc-800 rounded-xl p-1" role="tablist" aria-label="Authentication options">
          <button
            ...
            id="signin-tab"
            className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${mode === 'signin' ? 'bg-orange-500 text-white' : 'text-zinc-400'
              }`}
          >
            Sign In
          </button>
          <button
            ...
            id="signup-tab"
            className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${mode === 'signup' ? 'bg-orange-500 text-white' : 'text-zinc-400'
              }`}
          >
            Sign Up
          </button>
        ...
        {(error || isLimited) && (
          <div className="mb-4 p-3 bg-red-500/10 border border-red-500/30 rounded-lg text-red-400 text-sm" role="alert">
            {isLimited ? RATE_LIMITS.auth.message : error}
          </div>
        )}
