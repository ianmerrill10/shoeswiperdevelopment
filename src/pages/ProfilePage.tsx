...
import { FaCog, FaSignOutAlt, FaHeart, FaShoppingBag, FaShieldAlt, FaGem, FaBell, FaEdit } from 'react-icons/fa';
...
const ProfilePage: React.FC = () => {
  ...
  const loadUserData = useCallback(async () => {
    if (!profile && !profileLoading) {
      // If profile loaded but is null, user might not be logged in or error
      // But useProfile handles the fetch. If it's done and no profile, redirect?
      // We'll trust auth check below
    }
      ...
      <div className="bg-gradient-to-b from-orange-500/20 to-transparent p-6 pt-12">
        <div className="flex justify-between items-start mb-6">
          <div className="flex items-center gap-4">
            <div className="w-20 h-20 rounded-full bg-zinc-800 border-2 border-orange-500 overflow-hidden relative">
              {profile?.avatar_url ? (
              ...
              {profile?.style_tags && profile.style_tags.length > 0 && (
                <div className="flex gap-1 mt-2">
                  {profile.style_tags.slice(0, 3).map(tag => (
                    <span key={tag} className="text-[10px] bg-zinc-800 text-zinc-300 px-2 py-0.5 rounded-full">
                      {tag}
                    </span>
                  ))}
                </div>
      ...
      <div className="px-6 mb-4">
        ...
        >
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-full flex items-center justify-center ${pushEnabled ? 'bg-green-500/20' : 'bg-zinc-800'
              }`}>
          ...
          <div className={`px-2 py-1 rounded text-xs font-bold ${pushEnabled ? 'bg-green-500/20 text-green-400' : 'bg-orange-500/20 text-orange-400'
            }`}>
            {pushEnabled ? 'ON' : 'OFF'}
          </div>
        </button>
      ...
      <div className="sticky top-0 bg-zinc-950 border-b border-zinc-800 z-10" role="tablist" aria-label="Profile sections">
        <div className="flex px-6">
          <button
            ...
            id="favorites-tab"
            className={`flex-1 py-4 text-sm font-medium flex items-center justify-center gap-2 border-b-2 transition-colors ${activeTab === 'favorites'
          ...
          <button
            ...
            id="closet-tab"
            className={`flex-1 py-4 text-sm font-medium flex items-center justify-center gap-2 border-b-2 transition-colors ${activeTab === 'closet'
              ? 'text-orange-500 border-orange-500'
              : 'text-zinc-400 border-transparent'
              }`}
          >
